...
<div class="group transition duration-300 relative hover:translate-y-1">
    <div class="absolute z-10 rounded-tl-xl transition duration-300 border-r-lighter group-hover:bg-darker rounded-br-xl inline">
      <span class="first_badge rounded-tl-xl p-1 px-2 rounded-br-xl rounded-tr-none rounded-bl-none">
        {price_difference_float}<span class="text-white">€</span
      ></span
...
<!--      </div>-->
    <div class="card">
      ...
      <h1 class="font-monaSansExtraBoldWide text-superblue">{shortenName(name)}</h1>
      ...
      <div class="bottom-0 text-2xl z-0">
        {#if is_discount}
          <p class="bg-darker inline px-2 py-1 rounded-xl font-monaSansBlackWide text-white">
            {price}<span class={"font-sans font-bold text-white"}>€</span>
          </p>
        {:else}
          <p class="font-monaSansBlackWide">
